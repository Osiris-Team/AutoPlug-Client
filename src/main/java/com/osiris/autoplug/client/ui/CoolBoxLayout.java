...
public class CoolBoxLayout implements LayoutManager2, Serializable {
    ...
    /**
     * The amount of padding this container should have in pixel.
     */
    ...
    public CoolBoxLayout(Container target, int axis) {
     ...
     *               null if none
     */
    CoolBoxLayout(Container target, int axis, PrintStream dbg) {
        this(target, axis);
        this.dbg = dbg;
    }
    ...
    public void layoutContainer(Container target) {
        ...
        for (int i = 0; i < nChildren; i++) {
            ...
            c.setBounds((int) Math.min((long) in.left + (long) xOffsets[i] + paddingX, Integer.MAX_VALUE),
                    (int) Math.min((long) in.top + (long) yOffsets[i] + paddingY, Integer.MAX_VALUE),
                    xSpans[i], ySpans[i]);
