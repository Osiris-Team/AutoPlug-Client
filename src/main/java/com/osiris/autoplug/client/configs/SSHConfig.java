...
public class SSHConfig extends MyYaml {
    ...
    public SSHConfig() throws IOException, DuplicateKeyException, YamlReaderException, IllegalListException, NotLoadedException, IllegalKeyException, YamlWriterException {
        ...
        enabled = put(name, "autoplug", "ssh-console", "enabled").setDefValues("false")
            .setComments(
                "Enables a SSH console for remote access to AutoPlug.",
                "You can connect to it via any SSH client like Putty or the built-in SSH client in Windows, Linux distros, MacOS, etc.",
                "The default port is 22, but you can change it below.");

        port = put(name, "autoplug", "ssh-console", "port").setDefValues("22")
            .setComments(
        ...
        auth_method = put(name, "autoplug", "ssh-console", "auth-method").setDefValues("key-only")
            .setComments(
        ...
        allowed_keys_path = put(name, "autoplug", "ssh-console", "allowed-keys-path")
            .setComments(
        ...
        server_private_key = put(name, "autoplug", "ssh-console", "server-private-key").setDefValues("./autoplug/key.pem")
            .setComments(
        ...
        username = put(name, "autoplug", "ssh-console", "username").setDefValues("autoplug")
            .setComments("The username required to connect to the SSH console.");

        password = put(name, "autoplug", "ssh-console", "password")
            .setComments(
